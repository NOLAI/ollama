...
type llamaServerLaunchConfig struct {
	...
	rpcServers           string // comma-separated list of RPC endpoints, e.g. "host:port,host2:port2"
}
...
func collectRPCEndpoints(gpus []ml.DeviceInfo) string {
	...
	for _, gpu := range gpus {
		...
		endpoint := gpu.ID
		if i := strings.LastIndex(gpu.ID, ":"); i != -1 {
			if _, err := strconv.Atoi(gpu.ID[i+1:]); err == nil {
				endpoint = gpu.ID[:i]
			}
		}
		if !seen[endpoint] {
...
func NewLlamaServerRunner(
	...
	rpcServers := collectRPCEndpoints(gpus)
